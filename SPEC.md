...
Resolution is a single bit on a foreign thread, not an event log, so per-event tracking cannot express reopen cycles: exporters MUST reconcile it by state comparison — toggle on mismatch, and only when the latest local `resolve` event carries no foreign identity (a new local intent). A folded state that merely lags the forge — the latest local resolve was imported or already mirrored — is never pushed back. The mirror recorded for a toggle carries the foreign *thread* ID — the same ID import's synthetic resolve dedups on.
...
- **CLI** — the single-player wedge. The reference implementation covers `comment|reply|edit|delete|resolve|move|discard|show|list|status|seen|pull|commit|push|init|import|export` (search via `list --grep`, machine output via `--json`, GitHub and GitLab import and export per §8).
- **A `sync` command**: both forge pairs ship in the reference CLI, so bidirectional mirroring is `import` + `export` on a cadence — the convenience wrapper remains.
