...
- **CLI** — the single-player wedge. The reference implementation covers `comment|reply|edit|delete|resolve|move|discard|show|list|status|seen|pull|commit|push|init|import|export` (search via `list --grep`, machine output via `--json`, GitHub import and export per §8).
- **GitLab importer and exporter** (the GitHub pair ships in the reference CLI); with export (§8.2) in place, bidirectional sync is a polling loop, not new format.
